...
export class BranchComponent implements OnInit {
  branchData: any[] = []; // Array to store branch data
  orgData: any[] = [];
  showAddBranchForm: boolean = false;
  showEditBranchForm: boolean = false;
  selectedBranch: any;
 ...
 searchQuery: string = '';
  ...
  ngOnInit(): void {
    ...
    if (branchDataStr) {
      this.branchData = JSON.parse(branchDataStr);
    }
  }
  ...
  removeBranch(index: number): void {
    ...
    if (confirmation) {
      ...
      localStorage.setItem('branchData', JSON.stringify(this.branchData));
      this.snackBar.open('Branch deleted successfully!', 'Close', { duration: 2000 });
    }
  }
  get filteredBranches() {
    ...
    return this.branchData.filter(branch => branch.name.toLowerCase().includes(this.searchQuery.toLowerCase()));
 } 
  ...
  saveBranch(): void {
    ...
    newBranch.organization  = newBranch.organization;
  ...
  editBranch(index: number): void {
    this.selectedBranch = this.branchData[index];
    this.editBranchForm.patchValue({
      name: this.selectedBranch.name,
      state: this.selectedBranch.state,
      district: this.selectedBranch.district,
      address: this.selectedBranch.address,
      organization: this.selectedBranch.organization
    });
    this.showEditBranchForm = true;
  }

  saveEditedBranch(): void {
    if (this.editBranchForm.invalid) {
      return;
    }

    const editedBranch = this.editBranchForm.value;
    this.branchData[this.branchData.indexOf(this.selectedBranch)] = editedBranch;
    localStorage.setItem('branchData', JSON.stringify(this.branchData));
    this.showEditBranchForm = false;
    this.snackBar.open('Branch updated successfully!', 'Close', { duration: 2000 });
  }

  closeEditBranchForm(): void {
    this.showEditBranchForm = false;
  }
 ...
 'Andhra Pradesh': ['Anantapur', 'Chittoor', 'East Godavari', 'Guntur', 'Kadapa', 'Krishna', 'Kurnool', 'Prakasam', 'Sri Potti Sriramulu Nellore', 'Srikakulam', 'Visakhapatnam', 'Vizianagaram', 'West Godavari'],
 ...
 'Assam': ['Baksa', 'Barpeta', 'Biswanath', 'Bongaigaon', 'Cachar', 'Chirang', 'Darrang', 'Dhemaji', 'Dhubri', 'Dibrugarh', 'Goalpara', 'Golaghat', 'Hailakandi', 'Jorhat', 'Kamrup', 'Kamrup Metropolitan', 'Karbi Anglong', 'Karimganj', 'Kokrajhar', 'Lakhimpur', 'Majuli', 'Morigaon', 'Nagaon', 'Nalbari', 'Sivasagar', 'Sonitpur', 'Tinsukia', 'Udalguri'],
 'Bihar': ['Araria', 'Arwal', 'Aurangabad', 'Banka', 'Begusarai', 'Bhagalpur', 'Bhojpur', 'Buxar', 'Darbhanga', 'East Champaran', 'Gaya', 'Gopalganj', 'Jamui', 'Jehanabad', 'Kaimur', 'Katihar', 'Khagaria', 'Kishanganj', 'Lakhisarai', 'Madhepura', 'Madhubani', 'Munger', 'Muzaffarpur', 'Nalanda', 'Nawada', 'Patna', 'Purnia', 'Rohtas', 'Saharsa', 'Samastipur', 'Saran', 'Sheikhpura', 'Sheohar', 'Sitamarhi', 'Siwan', 'Supaul', 'Vaishali', 'West Champaran'],
 'Chandigarh': ['Chandigarh'],
 'Chhattisgarh': ['Balod', 'Baloda Bazar', 'Balrampur', 'Bastar', 'Bemetara', 'Bijapur', 'Bilaspur', 'Dakshin Bastar Dantewada', 'Dhamtari', 'Durg', 'Gariaband', 'Janjgir-Champa', 'Jashpur', 'Kabirdham', 'Kanker', 'Kawardha', 'Kondagaon', 'Korba', 'Koriya', 'Mahasamund', 'Mungeli', 'Narayanpur', 'Raigarh', 'Raipur', 'Rajnandgaon', 'Sukma', 'Surajpur', 'Surguja'],
 ...
 'Haryana': ['Ambala', 'Bhiwani', 'Charkhi Dadri', 'Faridabad', 'Fatehabad', 'Gurugram', 'Hisar', 'Jhajjar', 'Jind', 'Kaithal', 'Karnal', 'Kurukshetra', 'Mahendragarh', 'Nuh', 'Palwal', 'Panchkula', 'Panipat', 'Rewari', 'Rohtak', 'Sirsa', 'Sonipat', 'Yamuna Nagar'],
 'Himachal Pradesh': ['Bilaspur', 'Chamba', 'Hamirpur', 'Kangra', 'Kinnaur', 'Kullu', 'Lahaul and Spiti', 'Mandi', 'Shimla', 'Sirmaur', 'Solan', 'Una'],
 'Jammu and Kashmir': ['Anantnag', 'Bandipora', 'Baramulla', 'Budgam', 'Doda', 'Ganderbal', 'Jammu', 'Kathua', 'Kishtwar', 'Kulgam', 'Kupwara', 'Mirpur', 'Muzaffarabad', 'Pulwama', 'Punch', 'Rajouri', 'Ramban', 'Reasi', 'Samba', 'Shopian', 'Srinagar', 'Udhampur'],
 ...
 'Karnataka': ['Bagalkote', 'Ballari', 'Belagavi', 'Bengaluru Rural', 'Bengaluru Urban', 'Bidar', 'Chamarajanagar', 'Chikkaballapura', 'Chikkamagaluru', 'Chitradurga', 'Davanagere', 'Dharwad', 'Dakshina Kannada', 'Gadag', 'Hassan', 'Haveri', 'Kalaburagi', 'Kodagu', 'Kolar', 'Koppal', 'Mandya', 'Mysuru', 'Raichur', 'Ramanagara', 'Shivamogga', 'Tumakuru', 'Udupi', 'Uttara Kannada', 'Vijayapura', 'Yadgir'],
 'Kerala': ['Alappuzha', 'Ernakulam', 'Idukki', 'Kannur', 'Kasaragod', 'Kollam', 'Kottayam', 'Kozhikode', 'Malappuram', 'Palakkad', 'Pathanamthitta', 'Thiruvananthapuram', 'Thrissur', 'Wayanad'],
 'Ladakh': ['Leh', 'Kargil'],
 'Lakshadweep': ['Agatti', 'Kavaratti', 'Kadmat', 'Minicoy'],
 'Madhya Pradesh': ['Agar Malwa', 'Alirajpur', 'Anuppur', 'Ashoknagar', 'Balaghat', 'Barwani', 'Betul', 'Bhind', 'Bhopal', 'Burhanpur', 'Chhatarpur', 'Chhindwara', 'Damoh', 'Datia', 'Dewas', 'Dhar', 'Dindori', 'Guna', 'Gwalior', 'Harda', 'Hoshangabad', 'Indore', 'Jabalpur', 'Jhabua', 'Katni', 'Khandwa', 'Khargone', 'Mandla', 'Mandsaur', 'Morena', 'Narsinghpur', 'Neemuch', 'Niwari', 'Panna', 'Raisen', 'Rajgarh', 'Ratlam', 'Rewa', 'Satna', 'Sawai Madhopur', 'Sehore', 'Seoni', 'Shahdol', 'Shajapur', 'Sheopur', 'Shivpuri', 'Sidhi', 'Singrauli', 'Tikamgarh', 'Ujjain', 'Umaria', 'Una', 'Vidisha'],
 ...
 'West Bengal': ['Alipurduar', 'Bankura', 'Birbhum', 'Cooch Behar', 'Dakshin Dinajpur', 'Darjeeling', 'Hooghly', 'Howrah', 'Jalpaiguri', 'Jhargram', 'Kalimpong', 'Kolkata', 'Malda', 'Murshidabad', 'Nadia', 'North 24 Parganas', 'Paschim Bardhaman', 'Paschim Medinipur', 'Purba Bardhaman', 'Purba Medinipur', 'Purulia', 'South 24 Parganas', 'Uttar Dinajpur']
    ...
    this.districts = districtMap[state] || [];
}
...
canPerformAction(action: string): boolean {
  ...
  for (const organization in privileges) {
     ...
     if (organizationPrivileges.hasOwnProperty(userRole)) {
       ...
       if (rolePrivileges.hasOwnProperty('branchPrivileges') && rolePrivileges.branchPrivileges.hasOwnProperty(action) && rolePrivileges.branchPrivileges[action] === true) {
